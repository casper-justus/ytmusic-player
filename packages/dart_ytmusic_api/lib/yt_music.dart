...
class YTMusic {
  ...
  Future<dynamic> constructRequest(
    ...
    if (cookieString.isNotEmpty) {
      ...
      // Generate Authorization header for authenticated requests
      try {
        final sapisidMatch = RegExp(r'SAPISID=([^;]+)').firstMatch(cookieString);
        if (sapisidMatch != null) {
          ...
          headers['Authorization'] = 'SAPISIDHASH _';
        }
      } catch (e) {
        // Ignore auth generation errors
      }
    }
